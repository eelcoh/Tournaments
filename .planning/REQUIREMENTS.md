...
- [ ] **INST-01**: On iOS Safari, when the app is not installed (`navigator.standalone === false`), a dismissable banner appears at the bottom explaining how to add to Home Screen
- [ ] **INST-02**: On Android Chrome, when the `BeforeInstallPrompt` event fires, a dismissable banner appears at the bottom; tapping it triggers the native install dialog
- [ ] **INST-03**: Both banners match the terminal aesthetic (monospace, dark background, consistent with app style)
- [ ] **INST-04**: Dismissed state persists so the banner doesn't reappear on every visit
