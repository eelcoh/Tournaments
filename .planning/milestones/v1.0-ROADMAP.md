...
- PWA over native app — Elm SPA + vanilla JS service worker is zero-overhead installability (✓ Good)
- App-shell caching only (no offline data) — avoids sync complexity; fast load solves the main pain (✓ Good)
- Keep keyboard score input, improve layout — user prefers typing; focus on spacing/touch-target size (✓ Good)
- No skipWaiting() in SW — cache version bump (CACHE_NAME) is the correct update mechanism to avoid disrupting active tabs (✓ Good)
- inputmode=numeric (not type=number) for score inputs — avoids iOS/Android leading-zero stripping bugs (✓ Good)
- Invisible-wrapper pattern for tap zone expansion — terminal aesthetic (small text) stays visually intact, only hit area grows to 44px (✓ Good)
- viewingRound in WizardState, not BracketState — keeps navigation state co-located with wizard selections (✓ Good)
- UI.Page.container with spacing 24 vs page with spacing 20 — distinct rhythm for non-form pages (✓ Good)
- Form pages satisfy CON-01 via viewCardChrome (fill |> maximum Screen.maxWidth) — no migration needed, confirmed by human visual verification (✓ Good)

**Issues Resolved:**

- Bracket team flags missing from computer-layout team cells (regression from terminal redesign)
- No completion feedback when R32 is filled — added sticky inFront "Ga verder" button and checkmark stepper dots
- Topscorer page visually inconsistent — replaced badge buttons with terminal text rows
- Home page comment input unstyled — applied terminalInput styling with > prompt
- GroupBoundary rows different height from match rows — fixed with height (px 44) to prevent scroll wheel jumps

**Issues Deferred:**

- iOS Safari: no automatic install prompt — in-app tip needed (pass navigator.standalone as Elm flag) — future milestone
- iOS Safari: 7-day cache eviction — known constraint; do not architect features assuming persistent cache

**Technical Debt Incurred:**

- Any new static assets must be manually added to APP_SHELL in src/sw.js (no auto-discovery)
- UI.Style.terminalInput style not applied to future new Input.text calls — pattern must be maintained manually
